...
void test_fifo_evictor() {
    ...
    test_cache.set("key0", charr0, 711);  // I FORGET, DOES THIS NEED TO BE 711 CHARS + NULL TERMINATOR??? I THINK SO
    ...
    test_cache.set("key1", charr1, 498);
    uint32_t s = 711;
    assert(test_cache.get("key0", s) == nullptr);
    s = 498;
    assert(test_cache.get("key1", s) != nullptr);
    assert(test_cache.space_used() == 498);
    ...
    test_cache.set("key2", charr2, 296);
    assert(test_cache.get("key1", s) != nullptr);
    s = 296;
    assert(test_cache.get("key2", s) != nullptr);
    assert(test_cache.space_used() == 498+296);
    delete test_evictor;
}
